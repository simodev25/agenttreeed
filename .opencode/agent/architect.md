...
- `docs/architecture.md` and `docs/decision-pipeline.md` first, before recommendations
...
# Kairos Mesh architecture constraints

- Contraintes centrales: pipeline de décision 4 phases (Analyse → Débat → Décision → Gouvernance); ne pas remettre en cause cette structure sans demande explicite.
- Frontières critiques: `backend/app/risk/` (moteur de risque déterministe), `backend/app/services/metaapi/` (broker réel), APIs publiques FastAPI.
- Toute modification de frontière de service, modèle de données ou API publique nécessite ADR + validation humaine explicite.
