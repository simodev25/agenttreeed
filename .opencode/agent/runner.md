...
# Kairos Mesh command policy

- Commandes supportées en priorité:
  - `make backend-test`
  - `make frontend-build`
  - `make docker-up`
  - `cd backend && pytest -q`
...
- Classer les exécutions avant lancement:
  - **Rapide**: test ciblé/pytest unitaire
  - **Suite complète**: `make backend-test`, builds complets, E2E complets
  - **Dangereuse**: toute commande pouvant déclencher un ordre broker/MetaAPI live
- Interdiction stricte: ne jamais lancer une commande dangereuse (MetaAPI live/ordre réel) sans confirmation explicite du parent humain.
- Tous les outputs longs restent sous `.samourai/tmpai/` (jamais `/tmp`).
