...
- **Source de vérité tracker**: `.samourai/ai/agent/pm-instructions.md` est la référence unique pour la configuration GitHub Issues (repo, labels, transitions, fallback CLI).
- **Désambiguïsation stricte**: quand le développeur parle d'« agent », « prompt » ou « skill », il désigne les entités de trading (`backend/app/services/agentscope/agents.py`, prompts en base, `backend/config/skills/**`) — jamais les agents/skills OpenCode.
...
- **Validation humaine obligatoire**: confirmer avec l'humain avant toute livraison touchant `backend/app/risk/`, `backend/app/services/metaapi/`, ou un changement de posture `ALLOW_LIVE_TRADING`.
  </operating_principles>
...
- Lire les docs de cadrage avant de lancer une livraison: `docs/architecture.md`, `docs/decision-pipeline.md`, `docs/api-contracts-backend.md`
