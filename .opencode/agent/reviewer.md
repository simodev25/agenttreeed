...
**Project-critical checks (Kairos Mesh)**
- En cas de changement dans `backend/app/risk/`: exiger preuve d'approbation humaine explicite et justification documentaire.
- Vérifier l'alignement des contrats API avec `docs/api-contracts-backend.md` pour tout changement d'endpoint, payload ou rôle.
- Vérifier les migrations Alembic: cohérence modèle ↔ migration ↔ rollback minimal.
- Signaler toute confusion terminologique où "agent/prompt/skill" serait interprété comme OpenCode au lieu des entités trading.
- Vérifier la couverture de validation adaptée: pytest backend pour changements backend; Playwright pour impacts frontend/E2E.
